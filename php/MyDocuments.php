...
switch ($act){
    ...
    case 'deleteNote':
        ...
        while($row=$ret->fetchArray(SQLITE3_ASSOC)){
            $content = $row['content'];
            $db->query("insert into trashNotes(username,notebook,note,createTime,deleteTime,content) values ('".$username."','".$notebookName."','".$deleteNoteName."','".$createTime."','".$deleteTime."','".$content."')");
        }
        $exec1 = $db->query($sql1);
